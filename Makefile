...
## Builds and installs the plugin to a server through Cloudflare Tunnel.
.PHONY: deploy
deploy: dist
	@echo "Uploading plugin $(PLUGIN_ID) through tunnel..."
	curl --fail-with-body -X POST "$(MM_SERVICESETTINGS_SITEURL)/api/v4/plugins" \
	...
	curl --fail-with-body -X POST "$(MM_SERVICESETTINGS_SITEURL)/api/v4/plugins/$(PLUGIN_ID)/enable" \
		-H "Authorization: Bearer $(MM_ADMIN_TOKEN)" \
		-H "CF-Access-Client-Id: $(CF_CLIENT_ID)" \
		-H "CF-Access-Client-Secret: $(CF_CLIENT_SECRET)"
...
## The following targets (disable, enable, reset, kill, logs, logs-watch) require pluginctl
## installed locally (build/bin/pluginctl). They are local-development conveniences only
## and are not available in CI or the Cloudflare Tunnel deploy pipeline.
